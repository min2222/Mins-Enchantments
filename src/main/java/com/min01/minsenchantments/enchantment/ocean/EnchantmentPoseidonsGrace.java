...
public class EnchantmentPoseidonsGrace extends AbstractOceanEnchantment implements IProjectileEnchantment
{
	...
	{
		return this.getMinCost(pLevel) + EnchantmentConfig.poseidonsGraceMaxCost.get();
	}
	
	@Override
	public int getMinCost(int pLevel) 
	{
		return EnchantmentConfig.poseidonsGraceMinCost.get() + (pLevel - 1) * EnchantmentConfig.poseidonsGraceMaxCost.get();
	}
						...
						if(t.hasEnchantment(this))
						{
							...
							if(!tag.contains(EnchantmentTags.POSEIDONS_GRACE_SUMMONED))
							{
								...
								for(int i = 0; i < level * 2; i++)
								{
				                    ...
				                    {
				                    	ThrownTrident summonedTrident = EntityType.TRIDENT.create(projectile.level);
										...
										ObfuscationReflectionHelper.setPrivateValue(ThrownTrident.class, summonedTrident, ItemStack.of(tag.getCompound(EnchantmentTags.POSEIDONS_GRACE_ITEM)), "f_37555_");
										projectile.level.addFreshEntity(summonedTrident);
										summonedTrident.getCapability(EnchantmentCapabilityImpl.ENCHANTMENT).ifPresent(t2 -> 
										{
	...
	public void onEntityTick(Entity entity) 
	{
		...
		{
			trident.getCapability(EnchantmentCapabilityImpl.ENCHANTMENT).ifPresent(t -> 
			{
				if(t.hasEnchantment(this))
				{
					EnchantmentData data = t.getEnchantmentData(this);
					...
					if(tag.contains(EnchantmentTags.POSEIDONS_GRACE_SUMMONED))
					{
						if(trident.tickCount >= 20)
						{
							if(trident.level instanceof ServerLevel level)
							{
								...
								if(target != null)
								{
									trident.setDeltaMovement(EnchantmentUtil.getVelocityTowards(trident.position(), target.position().add(0, target.getEyeHeight(), 0), EnchantmentConfig.poseidonsGraceTridentSpeed.get()));
								}
								else
								{
									trident.discard();
